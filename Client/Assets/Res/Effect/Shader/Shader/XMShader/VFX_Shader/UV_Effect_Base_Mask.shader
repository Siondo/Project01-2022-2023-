Shader "SNKShader/VFX_Effect/UV_Effect_Base_Mask" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            fixed _MeshDepth;
            fixed _SoftHard;
            float4 _UIFXClipRange0 = float4(0.0, 0.0, 1.0, 1.0); // minx maxx miny maxy
            ...
            float4 frag(v2f i, float facing : VFACE) : SV_Target {
                ...
                float4 time3 = _Time;
                float2 maskPanner = (i.uv0+(float2(_MaskTexPannerX,_MaskTexPannerY)*time3.y));
                float4 _MaskTex_var = tex2D(_MaskTex,TRANSFORM_TEX(maskPanner, _MaskTex));
                float vAlpha = (_MainColor.a*(i.vertexColor.a*(_MainTex_var.a*_MaskTex_var.r)));
                #if UICLIP_ON
                // Softness factor
                ...
                #endif
                fixed MD = saturate(lerp( vAlpha, (vAlpha*pow(0.5*dot(normalDirection,viewDirection)+0.5,exp2(lerp(1,11,_MeshDepthIntensity)))), _MeshDepth ));
